...
export default function RestaurantsScreen ({ navigation, route }) {
  ...
  const renderRestaurant = ({ item }) => {
    return (
      ...
      >
        ...
        {/* Solución */}
        {item.promoted && (
          <TextRegular style={styles.promotedText}>¡En promoción!</TextRegular>
        )}
        </View>
  ...
  const promoteRestaurant = async (restaurant) => {
    try { // Siempre que se usa algún endpoint estaremos en una función asíncrona, y siempre que haya función asíncrona habrá try y catch
      await promote(restaurant.id)
      await fetchRestaurants() // Una vez actualizado tenemos que buscarlos para que se muestre la actualización
      setRestaurantToBePromoted(null) // Quitamos el modal
      showMessage({ // Mostramos el mensaje
...
const styles = StyleSheet.create({
  ...
  promotedText: {
    ...
    borderWidth: 3, // Para que se vea el borde
    borderRadius: 10,
    borderColor: GlobalStyles.brandSuccess,
    textAlign: 'center',
    paddingHorizontal: 10 // Para que el borde no afecte al texto
  }
})
